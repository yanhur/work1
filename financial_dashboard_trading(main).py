...
choices = ['台積電: 2022.01.03 至 2025.04.16', '大台指期貨: 2024.01.02 至 2025.03.14', '小台指期貨: 2024.01.02 至 2025.03.14', '華碩2023.04.17 至 2025.04.16', '堤維西2020.1.2 至 2024.4.12']
choice = st.selectbox('選擇金融商品', choices, index=0)
##### 读取Pickle文件
if choice == choices[0] :         ##'台積電: 2022.1.1 至 2024.4.9':
    df_original = load_data('kbars_1min_2330_2022-2025.pkl')
...
if choice == choices[1] :                   ##'大台指期貨2024.12到期: 2023.12 至 2024.4.11':
    df_original = load_data('kbars_1min_TXF_2024-2025.pkl')
    product_name = '大台指期貨'
if choice == choices[2] :                              ##'小台指期貨2024.12到期: 2023.12 至 2024.4.11':
    df_original = load_data('kbars_1min_MXF_2024-2025.pkl')
    product_name = '小台指期貨'
if choice == choices[3] :                                           ##'英業達2020.1.2 至 2024.4.12':
    df_original = load_data('kbars_1min_2357_2023-2025.pkl')
    product_name = '華碩2357'
...
if choice == choices[0] :                       ##'台積電: 2022.1.1 至 2024.4.9':
    start_date = st.text_input('輸入開始日期(日期格式: 2022-01.03), 區間:2022-01-03 至 2025-04-16', '2022-01-03')
    end_date = st.text_input('輸入結束日期 (日期格式: 2025-04-16), 區間:2022-01-03 至 2025-04-16', '2025-04-16')
if choice == choices[1] :                                   ##'大台指期貨2024.12到期: 2023.12 至 2024.4.11':
    start_date = st.text_input('輸入開始日期(日期格式: 2024-01-02), 區間:2024-01-02 至 2025-03-14', '2024-01-02')
    end_date = st.text_input('輸入結束日期 (日期格式: 2025-03-14), 區間:2024-01-02 至 2025-03-14', '2025-03-14')
if choice == choices[2] :                                               ##'小台指期貨2024.12到期: 2023.12 至 2024.4.11':
    start_date = st.text_input('輸入開始日期(日期格式: 2024-01-02), 區間:2024-01-02 至 2025-03-14', '2024-01-02')
    end_date = st.text_input('輸入結束日期 (日期格式: 2025-03-14), 區間:2024-01-02 至 2025-03-14', '2025-03-14')
if choice == choices[3] :                                                ##'英業達2020.1.2 至 2024.4.12':
...
with st.expander("長短 RSI"):
    ...
    fig2.add_trace(go.Scatter(x=KBar_df['time'][last_nan_index_RSI+1:], y=KBar_df['RSI_long'][last_nan_index_RSI+1:], mode='lines',line=dict(color='#00FFFF', width=2), name=f'{LongRSIPeriod}-根 K棒 移動 RSI'), 
                  secondary_y=False)
